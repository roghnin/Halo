...
#define CLHT_OCCUP_AFTER_RES 20
...
VMEM *vmp = vmem_create(PMEM_LOC.c_str(), PM_POOL_SZ);
...
static inline void clflush_next_check(char *data, int len, bool fence) {
  ...
  for (; ptr < data + len; ptr += kCacheLineSize) {
    ...
    if (((bucket_t *)data)->next)
      clflush_next_check((char *)(((bucket_t *)data)->next), sizeof(bucket_t),
                         false);
  }
  if (fence) mfence();
}
...
inline int clht_gc_collect(clht_t *hashtable) {
#if CLHT_DO_GC == 1
...
inline int clht_gc_release(clht_hashtable_t *hashtable) {
  ...
  for (bin = 0; bin < num_buckets; bin++) {
    ...
    while (bucket != NULL) {
      ...
      bucket = bucket->next;
      vmem_free(vmp, (void *)cur);
    }
  }
  vmem_free(vmp, hashtable->table);
  vmem_free(vmp, hashtable);
  return 1;
}
/* Create a new bucket. */
bucket_t *clht_bucket_create() {
  ...
  bucket =
      (bucket_t *)vmem_aligned_alloc(vmp, CACHE_LINE_SIZE, sizeof(bucket_t));
  if (bucket == NULL) {
    return NULL;
  }
...
clht_t *clht_create(uint64_t num_buckets) {
  ...
  clht_t *w =
      (clht_t *)vmem_aligned_alloc(vmp, CACHE_LINE_SIZE, sizeof(clht_t));
  if (w == NULL) {
...
clht_hashtable_t *clht_hashtable_create(uint64_t num_buckets) {
  ...
  hashtable = (clht_hashtable_t *)vmem_aligned_alloc(vmp, CACHE_LINE_SIZE,
  ...
  hashtable->table = (bucket_t *)vmem_aligned_alloc(
      vmp, CACHE_LINE_SIZE, num_buckets * (sizeof(bucket_t)));
...
static inline bool bucket_exists(volatile bucket_t *bucket, clht_addr_t key) {
  uint32_t j;
  do {
    for (j = 0; j < ENTRIES_PER_BUCKET; j++) {
      if (bucket->key[j] == key) {
...
static int bucket_cpy(clht_t *h, volatile bucket_t *bucket,
...
int ht_resize_pes(clht_t *h, int is_increase, int by) {
  ...
  resizingPtr = (char *)ht_new;
  clflush((char *)&resizingPtr, sizeof(char *), false);
  ...
  clflush_next_check((char *)ht_new->table, num_buckets_new * sizeof(bucket_t),
                     false);
  mfence();
...
// hash_Utilization utiliz(clht_t *h) {
//   clht_hashtable_t *hashtable = h->ht;
//   uint64_t num_buckets = hashtable->num_buckets;
//   uint64_t nb = 0;
//   volatile bucket_t *bucket = NULL;
...
//     } while (bucket != NULL);
//   }
//   // std::cout << size << " " << (hashtable->num_buckets) *
//   ENTRIES_PER_BUCKET
//   //           << " "
//   //           << 100.0 * size / ((hashtable->num_buckets) *
//   ENTRIES_PER_BUCKET)
//   //           << std::endl;
//   hash_Utilization hu;
//   hu.load_factor = 100.0 * (float)size / (nb * ENTRIES_PER_BUCKET);
//   hu.utilization = 100.0 * (float)size * 16 / (nb * sizeof(bucket_t));
//   return hu;
// }
// returns 0 on crash
size_t ht_status(clht_t *h, int resize_increase, int just_print) {
  ...
  if (just_print) {
    ...
    if (full_ratio > 0 && full_ratio < CLHT_PERC_FULL_HALVE) {
      //            printf("[STATUS-%02d] #bu: %7zu / #elems: %7zu / full%%:
      //            %8.4f%% / expands: %4d / max expands: %2d\n",
      //                    clht_gc_get_id(), hashtable->num_buckets, size,
      //                    full_ratio, expands, expands_max);
